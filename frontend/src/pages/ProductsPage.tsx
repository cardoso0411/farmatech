...
import { productCategoryOptions, productGroupOptions } from '../data/productSupportData';
...
export function ProductsPage() {
  ...
  const sngpcOptions = useMemo(
    () => Array.from(new Set(productGroupOptions.map((item) => item.sngpc))),
    []
  );

  const saleOperationOptions = useMemo(
    () => Array.from(new Set(productGroupOptions.map((item) => item.saleOperation))),
    []
  );
  ...
  function selectPis(type: PisType) {
    ...
    setFormData((current) => ({
      ...current,
      pisPositive: type === 'POSITIVA',
      pisNegative: type === 'NEGATIVA',
      pisNeutral: type === 'NEUTRA',
    }));
  }
  ...
  return (
    <Box>
      <PageHeader
        title="Produtos"
        description="Cadastro de remédios com categorias, grupos, dados fiscais, estoque, flags e operação de venda."
      />
            ...
            <CardContent>
              <Stack spacing={3}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid size={{ xs: 12, md: 4 }}>
                    <TextField label="Resumo" fullWidth value={formData.summary} onChange={(event) => updateField('summary', event.target.value)} />
                  </Grid>
                  <Grid size={{ xs: 12, md: 4 }}>
                    <TextField
                      select
                ...
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12 }}>
                    <TextField label="Descrição" fullWidth value={formData.description} onChange={(event) => updateField('description', event.target.value)} />
                  </Grid>
                </Grid>
                ...
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, md: 2 }}>
                    <TextField label="Unidade" fullWidth value={formData.unit} onChange={(event) => updateField('unit', event.target.value)} />
                  </Grid>
                  <Grid size={{ xs: 12, md: 3 }}>
                ...
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, md: 4 }}>
                    <TextField label="Preço de venda" fullWidth value={formData.salePrice} onChange={(event) => updateField('salePrice', event.target.value)} />
                  </Grid>
                  <Grid size={{ xs: 12, md: 4 }}>
                ...
                <Box>
                  ...
                  <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} flexWrap="wrap" useFlexGap>
                    <FormControlLabel
                      control={<Checkbox checked={pisType === 'POSITIVA'} onChange={() => selectPis(pisType === 'POSITIVA' ? '' : 'POSITIVA')} />}
                      label="Positiva"
                    />
                    <FormControlLabel
                      control={<Checkbox checked={pisType === 'NEGATIVA'} onChange={() => selectPis(pisType === 'NEGATIVA' ? '' : 'NEGATIVA')} />}
                      label="Negativa"
                    />
                    <FormControlLabel
                      control={<Checkbox checked={pisType === 'NEUTRA'} onChange={() => selectPis(pisType === 'NEUTRA' ? '' : 'NEUTRA')} />}
                      label="Neutra"
                    />
                  </Stack>
                </Box>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                  <Button variant="contained">Gravar produto</Button>
